...
import { Activity, Lock, Mail, Eye, EyeOff, CheckCircle, UserPlus } from 'lucide-react';
...
export default function Login() {
  const { login, signUp, signUpWithToken } = useAuth();
  ...
  useEffect(() => {
    ...
    setMode('invite');
    supabase.from('invitations').select('email, display_name, provider_name, accepted_at, expires_at').eq('token', token).maybeSingle().then(({ data }) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if ((mode === 'signup' || mode === 'invite') && password !== confirmPassword) { setError('Passwords do not match.'); return; }
    if ((mode === 'signup' || mode === 'invite') && password.length < 8) { setError('Password must be at least 8 characters.'); return; }
    setLoading(true);
    try {
      if (mode === 'signin') {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-4">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-slate-500 text-sm leading-relaxed">
            We sent a verification link to <span className="font-medium text-slate-700">{doneEmail}</span>.
            Click the link to activate your account, then return here to sign in.
          </p>
          <button onClick={() => { setDone(false); switchMode('signin'); }} className="mt-8 text-sm text-blue-600 hover:text-blue-700 underline underline-offset-2 transition-colors">
            Back to sign in
          </button>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
          {isInviteMode ? (
              ...
              {inviteError ? (
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{inviteInfo.display_name}{inviteInfo.provider_name ? ` — ${inviteInfo.provider_name}` : ''}</p>
                  </div>
                </div>
              ) : (
            ...
            <div className="flex rounded-lg bg-slate-100 p-1 mb-6">
              <button type="button" onClick={() => switchMode('signin')} className={`flex-1 text-sm font-medium py-1.5 rounded-md transition-all duration-150 ${mode === 'signin' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Sign in</button>
              <button type="button" onClick={() => switchMode('signup')} className={`flex-1 text-sm font-medium py-1.5 rounded-md transition-all duration-150 ${mode === 'signup' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Create account</button>
            </div>
          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required readOnly={isInviteMode && !!inviteInfo}
              ...
              <button type="submit" disabled={loading || (isInviteMode && (!inviteInfo || !!inviteError))}
                ...
                {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />{mode === 'signin' ? 'Signing in...' : 'Creating account...'}</> : mode === 'signin' ? 'Sign in' : isInviteMode ? 'Activate account' : 'Create account'}
              </button>
            </form>
          )}
          {mode === 'signup' && <p className="mt-4 text-xs text-slate-400 text-center leading-relaxed">Only pre-authorized email addresses can create accounts. Contact your administrator if you need access.</p>}
        </div>
        <p className="text-center text-xs text-slate-400 mt-6">BHAC IDR Revenue Reporting System &copy; {new Date().getFullYear()}</p>
      </div>
    </div>
  );
}
